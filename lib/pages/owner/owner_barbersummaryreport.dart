...
class OwnerBarberSummaryReport extends StatefulWidget {
  const OwnerBarberSummaryReport({super.key, required this.barberid});

  final String barberid;

  @override
  State<OwnerBarberSummaryReport> createState() =>
      _OwnerBarberSummaryReportState();
}

class _OwnerBarberSummaryReportState extends State<OwnerBarberSummaryReport> {
    ...
    try {
      for (String checkdate in lastSevenDays) {
        ...
        var bookedDateSnapshot = await FirebaseFirestore.instance
            .collection("Bookings")
            .where("barber_id", isEqualTo: widget.barberid)
